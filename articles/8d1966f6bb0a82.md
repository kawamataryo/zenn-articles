...
Twitterの場合は、API経由でもURLを本文に含めればよしなに展開してTwitterカードを作ってくれるのですが、BlueskyのAT Protocolの場合はそんな単純にはいきませんでした😅
...
当初その仕様でどのようにリクエストするのかわからず詰まったのですが、自分がBlueskyのアプリ経由でリンクカードを投稿したpostのLexiconを確認することで、把握しました。
...
重要なのは、`embed.thumb.ref.$link` でここに画像の参照リンクを指定する必要があります。この参照リンクにはBlueskyのPDSに画像をアップロードした際に取得できるリンクを指定します。
...
あとBlueskyアカウトはこちらなのでよければフォローしてください〜
