...
Property Based Testing は、Haskell のライブラリ [QuickCheck](https://hackage.haskell.org/package/QuickCheck) から始まったテストスタイルで、定義された条件に合わせて自動生成された膨大な値に対してテストを行う手法です。

Property Based Testing のライブラリに入力値の条件を渡すと、その条件からランダムな複数のテストケースを自動生成し実行してくれます。

短いコードで膨大なテストケースを検証できる点、テストケースが開発者に依存せず意図していないエッジケースを発見できる可能性がある点が優れています。
...
この関数を愚直にテストしようとした場合のテストコードがこちらです。
