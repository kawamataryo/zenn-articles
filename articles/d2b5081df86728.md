---
title: "Kotlin + GitHub Template ではじめる初めての IntelliJ IDEA プラグイン開発"
emoji: "🐙"
...
このリポジトリがなかなか高機能でリポジトリの作成が完了すると GitHub Actions が起動し、リポジトリ名を元にテンプレートコードの初期化が行われ、直ぐプラグインを起動できる状態となります（README も自動で書き換えられます）。
...
アクションは`AnAction`クラスを継承して作成します。オーバーライドしている`actionPerformed`がアクションが実行された際に動作するメソッドです。`actionPerformed`が引数として受け取る`AnActionEvent`のインスタンスから File やプロジェクトの情報を取得できます。今回はそこから Root からファイルまでのパスを取得し、clipboard へコピーしています。
...
すると別のプラグインがインストールされた状態の別の IntelliJ が起動します。適当なファイルを開き、コンテキストメニューを開くと先程定義した`Copy Path From Root`が表示されてます。
...
実際にプラグインを公開する場合は、`gradle.properties`にバージョン情報や、対応プラットフォームを設定します。この設定が、Gradle でのビルド時に読み込まれて、プラグインの jar ファイルが作成されます。
